...
### 三方验证
若在本地部署的dify安装此插件，可能会遇到验证失败的情况。为此，请通过dify工具申请公钥/私钥，对difypkg进行加密，在配置文件中添加允许三方验证和密钥存放地址，存放好密钥后再将加密过的difypkg上传安装
...
```
### 提示
当LLM无法正确理解或处理参数时，可以在您的提示词中添加parameter_examples.md文件中的示例来辅助模型推理。这些示例提供了标准格式的参数输入，有助于模型更准确地理解您的需求。
